...
export const LeadProfileInfo = createAsyncThunk(
    "/lead/dashboard",
    async () => {
        const response = await fetch("https://cms-backend-rho.vercel.app/lead/profile/info", {
            credentials: "include"
        })
...
export const UpdateLeadProfileInfo = createAsyncThunk(
    ...
    async (formData: IUpdateProfileData) => {
        const response = await fetch("https://cms-backend-rho.vercel.app/lead/profile/update", {
            credentials: "include",
            method: "PUT",
            headers: {
